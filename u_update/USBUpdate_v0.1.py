...
def A13YConnect():
    try:
        ...
        time.sleep(1)
        if d.xpath(
                '//*[@resource-id="com.gxa.service.systemui:id/state_bar_signal_layout"]/android.widget.FrameLayout[1]').exists:
            d.xpath(
                '//*[@resource-id="com.gxa.service.systemui:id/state_bar_signal_layout"]/android.widget.FrameLayout[1]').click()
        time.sleep(1)
        if d(resourceId="com.gxatek.cockpit.account:id/tv_experience").exists:
            d(resourceId="com.gxatek.cockpit.account:id/tv_experience").click()
        time.sleep(1)
        Disconnect = (d(resourceId="com.gxa.service.systemui:id/btnDisconnect", text="断开连接").exists)
        btnIgnore = (d(resourceId="com.gxa.service.systemui:id/btnIgnore", text="忽略").exists)
        if Disconnect == True or btnIgnore == True:
            d(resourceId="com.gxa.service.systemui:id/title", text="中瓴智行").click()
            d(resourceId="com.gxa.service.systemui:id/et_input").send_keys("66666666")
            d(resourceId="com.gxa.service.systemui:id/btn_positive").click()
        Logger.info("WiFi successfully connected")
    except Exception as e:
...
def getDvicesIP():
    try:
        time.sleep(2)
        with os.popen(r'adb -s 1234567 shell ifconfig | findstr "192.168.12"', 'r') as f:
            text = f.read()
...
download = Download()
# 远程文件路径（需要绝对路径）
remote_dir_name = '/home/share/ci_deploy/gq8155/20220608/output/GACXXXX_A13Y_AVNT_SH_220608.R8.02_D.zip'
# 本地文件存放路径（绝对路径或者相对路径都可以）
local_dir_name = 'E:/test_version'

# 远程文件开始下载
download.down_from_remote(remote_dir_name, local_dir_name)
